...
def _process_text(
    ...
    *,
    ...
    """Stage-1 text path: batched ``run_text_batch`` over ``spec.clip_input`` evidence."""
    schema = clip_schema(spec, labels)
    mir_by_clip, visual_by_clip, dep_terminal = _text_evidence_maps(
        session, pending=pending, spec=spec, labels=labels
    )
    # Phase 1: resolve evidence; write terminal-failure rows for unusable clips.
    ready: list[tuple[int, tuple, str]] = []  # (clip_id, key, prompt)
    for clip_id in pending:
        key = (clip_id, spec.name)
        ...
        if evidence is None:
            ...
            continue
        ready.append((clip_id, key, f"{prompt_body}\n\n{evidence}"))
    session.commit()

    # Phase 2: batched generation in chunks (vLLM pipelines within each chunk).
    for start in range(0, len(ready), _TEXT_CLIP_BATCH_SIZE):
        chunk = ready[start : start + _TEXT_CLIP_BATCH_SIZE]
        prompts = [p for (_cid, _key, p) in chunk]
        label = f"{spec.name}/batch[{start}:{start + len(chunk)}]"
        with item("EXTRACT", label):
            try:
                raws = generator.run_text_batch(
                    prompts,
                    max_new_tokens=labels.max_new_tokens_for(spec.name),
                    seeds=[None] * len(prompts),
                    schema=schema,
                )
            except Exception as exc:  # whole-chunk runtime failure
                for _cid, key, _p in chunk:
                    bump_failure(
                        session,
                        ClipLabel,
                        key=key,
                ...
                session.commit()
                continue
            for (_cid, key, _p), raw in zip(chunk, raws, strict=True):
