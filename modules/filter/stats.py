...
def _build_user_stats(user: User, clips: list) -> UserStats:
    """Compute the UserStats row for one user's preprocessed clips."""
    ...
    return UserStats(
        ...
        log_plays_std=(
            float(statistics.pstdev(log_plays_vals)) if len(log_plays_vals) > 1 else 0.0
        ),
        ...
        mean_video_duration=(float(statistics.mean(durations)) if durations else None),
...
def calculate_user_stats(session: Session) -> None:
    ...
    for user in session.query(User).all():
        ...
        if not clips:
            continue
        session.add(_build_user_stats(user, clips))
