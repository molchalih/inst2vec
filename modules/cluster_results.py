...
def summarize_case_rows(rows: list[ClusterRun]) -> dict[str, str]:
    filtered_rows = [
        r for r in rows
        if r.disqualified != 1 and r.in_current_grid != 0
    ]
    dbcv_vals = [float(r.dbcv) for r in rows if r.dbcv is not None and math.isfinite(float(r.dbcv))]
    sil_vals = [float(r.silhouette) for r in rows if r.silhouette is not None and math.isfinite(float(r.silhouette))]
    k_vals = [float(r.n_clusters) for r in rows if r.n_clusters is not None]
    noise_pct_vals = [float(r.noise_ratio) * 100.0 for r in rows if r.noise_ratio is not None and math.isfinite(float(r.noise_ratio))]
