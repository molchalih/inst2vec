...
def test_aggregate_single_clip_per_user():
    rows = [
        (10, _make_blob([1.0, 2.0, 3.0]), 101),
        (11, _make_blob([4.0, 5.0, 6.0]), 102),
    ]
...
def test_aggregate_mean_of_multiple_clips():
    rows = [
        (10, _make_blob([1.0, 3.0]), 101),
        (11, _make_blob([3.0, 1.0]), 101),
        (12, _make_blob([0.0, 0.0]), 101),
    ]
    result = aggregate_user_embeddings_from_rows(rows)
...
def test_aggregate_output_dtype_is_float32():
    rows = [(10, _make_blob([1.0, 2.0]), 101)]
    result = aggregate_user_embeddings_from_rows(rows)
...
def test_aggregate_mixed_users():
    rows = [
        (10, _make_blob([2.0, 4.0]), 1),
        (11, _make_blob([0.0, 0.0]), 1),
        (20, _make_blob([10.0, 10.0]), 2),
    ]
    result = aggregate_user_embeddings_from_rows(rows)
